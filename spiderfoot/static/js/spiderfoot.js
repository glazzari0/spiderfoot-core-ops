...
document.addEventListener("DOMContentLoaded", () => {
  ...
  themeToggler.addEventListener("click", () => {
    ...
    if (localStorage.getItem("theme") === "dark-theme") {
      ...
      localStorage.setItem("mode", "Modo Escuro");
      link.rel = "stylesheet";
      link.type = "text/css";
      ...
      localStorage.setItem("mode", "Modo Claro");
      link.rel = "stylesheet";
      link.type = "text/css";
...
sf.deleteScan = function(scan_id, callback) {
    ...
    req.fail(function (hr, textStatus, errorThrown) {
        ...
        sf.log("Erro ao excluir varreduras: " + scan_id + ": " + hr.responseText);
    });
};
...
sf.stopScan = function(scan_id, callback) {
    ...
    req.done(function() {
        alertify.success('<i class="glyphicon glyphicon-ok-circle"></i> <b>Varreduras Abortadas</b><br/><br/>' + scan_id.replace(/,/g, "<br/>"));
        sf.log("Varreduras abortadas: " + scan_id);
        callback();
    });
    req.fail(function (hr, textStatus, errorThrown) {
        alertify.error('<i class="glyphicon glyphicon-minus-sign"></i> <b>Erro</b><br/><br/>' + hr.responseText);
        sf.log("Erro ao parar varreduras: " + scan_id + ": " + hr.responseText);
    });
};
...
sf.fetchData = function (url, postData, postFunc) {
  ...
  req.fail(function (hr, status) {
      alertify.error('<i class="glyphicon glyphicon-minus-sign"></i> <b>Erro</b><br/>' + status);
  });
};
