...
class SpiderFootDb:
    ...
    operationalSchemaQueries = [
        "CREATE TABLE IF NOT EXISTS tbl_scan_finding_state ( \
            scan_instance_id    VARCHAR NOT NULL REFERENCES tbl_scan_instance(guid), \
            result_hash         VARCHAR NOT NULL REFERENCES tbl_scan_results(hash), \
            triage_status       VARCHAR NOT NULL DEFAULT 'novo', \
            relevance           VARCHAR NOT NULL DEFAULT 'pendente', \
            exploitability      VARCHAR NOT NULL DEFAULT 'nao_avaliado', \
            analyst_verdict     VARCHAR NOT NULL DEFAULT 'em_analise', \
            notes               TEXT DEFAULT '', \
            updated             INT NOT NULL DEFAULT 0, \
            PRIMARY KEY (scan_instance_id, result_hash) \
        )",
        "CREATE TABLE IF NOT EXISTS tbl_scan_finding_evidence ( \
            id                  VARCHAR NOT NULL PRIMARY KEY, \
            scan_instance_id    VARCHAR NOT NULL REFERENCES tbl_scan_instance(guid), \
            result_hash         VARCHAR NOT NULL REFERENCES tbl_scan_results(hash), \
        ...
        "CREATE TABLE IF NOT EXISTS tbl_scan_validation_run ( \
            id                  VARCHAR NOT NULL PRIMARY KEY, \
            scan_instance_id    VARCHAR NOT NULL REFERENCES tbl_scan_instance(guid), \
            result_hash         VARCHAR NOT NULL REFERENCES tbl_scan_results(hash), \
        ...
        "CREATE TABLE IF NOT EXISTS tbl_scan_case_verdict ( \
            scan_instance_id    VARCHAR NOT NULL PRIMARY KEY REFERENCES tbl_scan_instance(guid), \
            verdict             VARCHAR NOT NULL DEFAULT 'em_analise', \
    ...
    def __init__(self, opts: dict, init: bool = False) -> None:
        ...
        with self.dbhLock:
            ...
            try:
                ...
                raise IOError("SpiderFoot wasn't able to initialize the operational investigation schema.") from e
        ...
        with self.dbhLock:
            try:
                for qry in self.createSchemaQueries:
                    self.dbh.execute(qry)
                for qry in self.operationalSchemaQueries:
                    self.dbh.execute(qry)
                self.conn.commit()
                for row in self.eventDetails:
                    event = row[0]
    ...
    def findingStateGet(self, instanceId: str, resultHash: str) -> dict:
        ...
        qry = "SELECT triage_status, relevance, exploitability, analyst_verdict, notes, updated \
            FROM tbl_scan_finding_state WHERE scan_instance_id = ? AND result_hash = ?"
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when fetching finding state") from e

        if not row:
            return {
                "triage_status": "novo",
                "relevance": "pendente",
                "exploitability": "nao_avaliado",
                "analyst_verdict": "em_analise",
                "notes": "",
                "updated": 0
            }
        ...
        return {
            ...
            "analyst_verdict": row[3],
            "notes": row[4] or "",
            "updated": row[5]
        }

    def findingStateSet(self, instanceId: str, resultHash: str, triageStatus: str, relevance: str,
                        exploitability: str, analystVerdict: str, notes: str) -> bool:
        if not isinstance(instanceId, str):
            raise TypeError(f"instanceId is {type(instanceId)}; expected str()") from None
        if not isinstance(resultHash, str):
            raise TypeError(f"resultHash is {type(resultHash)}; expected str()") from None

        qry = "REPLACE INTO tbl_scan_finding_state \
            (scan_instance_id, result_hash, triage_status, relevance, exploitability, analyst_verdict, notes, updated) \
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        qvars = [instanceId, resultHash, triageStatus, relevance, exploitability, analystVerdict, notes or "", int(time.time())]
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when storing finding state") from e

    def findingEvidenceList(self, instanceId: str, resultHash: str) -> list:
        qry = "SELECT id, evidence_type, title, content, created FROM tbl_scan_finding_evidence \
            WHERE scan_instance_id = ? AND result_hash = ? ORDER BY created DESC"
        qvars = [instanceId, resultHash]
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when fetching evidence") from e

    def findingEvidenceAdd(self, instanceId: str, resultHash: str, evidenceType: str, title: str, content: str) -> str:
        evidenceId = str(uuid.uuid4())
        qry = "INSERT INTO tbl_scan_finding_evidence \
            (id, scan_instance_id, result_hash, evidence_type, title, content, created) \
            VALUES (?, ?, ?, ?, ?, ?, ?)"
        qvars = [evidenceId, instanceId, resultHash, evidenceType, title, content, int(time.time())]
    ...
    def validationRunAdd(self, instanceId: str, resultHash: str, validator: str, status: str, summary: str, details: str) -> str:
        ...
        qry = "INSERT INTO tbl_scan_validation_run \
            (id, scan_instance_id, result_hash, validator, status, summary, details, created) \
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        qvars = [runId, instanceId, resultHash, validator, status, summary, details, int(time.time())]
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when storing validation run") from e

    def validationRunList(self, instanceId: str, resultHash: str) -> list:
        qry = "SELECT id, validator, status, summary, details, created FROM tbl_scan_validation_run \
            WHERE scan_instance_id = ? AND result_hash = ? ORDER BY created DESC"
        qvars = [instanceId, resultHash]
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when fetching validation runs") from e

    def caseVerdictGet(self, instanceId: str) -> dict:
        qry = "SELECT verdict, summary, updated FROM tbl_scan_case_verdict WHERE scan_instance_id = ?"
        qvars = [instanceId]
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when fetching case verdict") from e

        if not row:
            return {"verdict": "em_analise", "summary": "", "updated": 0}

        return {"verdict": row[0], "summary": row[1] or "", "updated": row[2]}

    def caseVerdictSet(self, instanceId: str, verdict: str, summary: str) -> bool:
        qry = "REPLACE INTO tbl_scan_case_verdict (scan_instance_id, verdict, summary, updated) VALUES (?, ?, ?, ?)"
        qvars = [instanceId, verdict, summary or "", int(time.time())]
    ...
    def scanInstanceDelete(self, instanceId: str) -> bool:
        ...
        qry5 = "DELETE FROM tbl_scan_finding_state WHERE scan_instance_id = ?"
